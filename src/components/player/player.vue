				...
				<scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
					<div class="lyric-wrapper">
						<div v-if="currentLyric">
							<p ref="lyricLine" :class="{'current' : currentLineNumber===index}" class="text" v-for="(line,index) in currentLyric.lines">
								{{line.txt}}
							</p>
						</div>
					</div>
...
<script>
/*因为要控制播放器的显示隐藏，所以要从vuex中读到数据进行判断*/
	...
	export default {
		...
		methods:{
			...
			next(){
				...
				// 	return;
				// }
				//只有一首歌的时候currentsong的id不变，后面的逻辑不执行
				if(this.playList.length===1){
					this.loop();
					return;
				}else{
			...
			_getPosAndScale(){
				const targetWidth=40;  //mini图大小
				const paddingLeft=40;  //mini图padding
				const paddingBottom=30;
				const paddingTop=80;   //mormal图的padding
				const width=window.innerWidth * 0.8; //normal图的宽度
...
.player{
	.normal-player{
		...
		&.normal-enter,&.normal-leave-to{
			...
			.middle .cdWrapper{
				transfrom:translate3d(0,0,-100px)
			}
		}
